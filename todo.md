# 军团审计插件：2.1 / 2.1.1 已发布与开发分支状态

> 已发布范围：正式标签 `2.1` 包含军团审查 2.0、令牌审查既有只读页面及 CSV；`2.1.1` 修复了筛选后误判主角色不在当前军团范围的问题。
>
> 开发分支范围：提交 `418f8d8` 在 `feature/corporation-audit-2.0` 增加令牌审查「最后离线」列、`last_logoff` 筛选和 CSV 列；它已在测试服务器验收，但尚未纳入正式标签，也不得因验收通过自动更新生产。
>
...
- [x] 已在 README 固化 schema contract、三态规则、只读边界、UTC 规则，以及入团/最后上线/最后离线三种独立来源、CSV 导出安全边界、技能占位、实时查询性能决策和 SeAT 升级后的 preflight 要求。
