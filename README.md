...
- **白名单豁免** — 两级白名单（角色 + 军团）。豁免语义：
    - **钱包审计**：发起方在角色白名单 → 豁免（对手方是市场）
    - **合同审计**：发起方 AND 接收方都「在白名单」（个人在角色白名单 OR 当前所属军团在军团白名单）→ 豁免；任一方在白名单外即审计（典型用例：内部主公司进军团白名单 → 内部互转豁免，内部与外部之间的合同仍审计）
...
**豁免判定（AND 语义）**：合同审计中，发起方 AND 接收方 都「在白名单」才整份合同跳过。单方「在白名单」= 个人在角色白名单 OR 当前所属军团在军团白名单。
- 把内部主公司（如 YeLuo-XingHai）加进军团白名单 → 内部成员之间的合同豁免，内部与外部之间的合同仍审计。
- 把信任的盟友军团加进 → 盟友间相互合同豁免。
...
- **审计优先级**：
    ...
    - 合同：仅 finished + item_exchange/auction → 双方都「在白名单」（角色或军团）AND 豁免；否则按物品匹配判定（不再单独检查 assignee）
